-- ============================================================
-- Multimoeda — dashboard_cc_agg com p_slot (mesma convenção da v3_070).
--
-- A v3_047 (dashboard_cc_agg) somava sempre `valor` (base/BRL). Agora aceita
-- p_slot int DEFAULT 1 e soma a coluna do slot pedido:
--   orçado  → fat_orcado.val_m<slot> (val_m1 = valor)
--   realiz. → fat_realizado_mensal.val_m<slot> (× sinal na resolução por conta)
...
AS $$
  ...
  r AS (
    SELECT cc_id, linha_id, ano, mes, sum(v) v FROM (
      ...
      SELECT m.cc_id, m.linha_id, m.ano, m.mes,
      ...
      SELECT m.cc_id, cl.linha_id, m.ano, m.mes,
