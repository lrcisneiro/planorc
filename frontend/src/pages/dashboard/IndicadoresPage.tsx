  ...
  return (
    <div style={S.page}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 2 }}>
        ...
        <h1 style={S.title}>Indicadores{cardNome && <span style={{ color: '#0c8599' }}> · {cardNome}</span>}</h1>
      </div>
      ...
      <div style={S.bar}>
        ...
        <PeriodoButton width="min(420px, calc(100vw - 40px))" resumo={`${ano} · até ${MESES[ateMes - 1]}`}>
          ...
          <label style={{ fontSize: 12, fontWeight: 500, color: '#495057', display: 'block', marginBottom: 6 }}>Acumulado até o mês</label>
        ...
        <SalvarCardButton base="/dashboards/indicadores" cor="#0c8599" cardId={cardId} getFiltros={() => ({ relId, versaoId, ano, ateMes, sel, empresaSel, filialSel, ccSel, areaSel, divisaoSel, buSel })} />
      </div>
    ...
    <div style={S.kpi}>
      ...
      <div style={S.ksub}>
        Orçado {fmt(c.O)} · {c.isPct
          ...
          : (exec == null ? '—' : <span style={{ color: bomExec ? '#2f9e44' : '#e03131', display: 'inline-flex', alignItems: 'center', gap: 3 }}>{arrow(bomExec)}{exec.toFixed(0)}%</span>)}
      </div>
      ...
      <div style={{ ...S.ksub, marginTop: 6, paddingTop: 6, borderTop: '1px solid #f1f3f5' }}>
        <strong style={{ color: '#495057' }}>{anoPrev}</strong> {fmt(c.P)} · {c.isPct
