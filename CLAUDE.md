...
- **F5.2 — Posto de trabalho (folha)** — planejado e aprovado (jul/2026): design completo em `docs/DESIGN_posto_trabalho.md` (grão híbrido funcionário+vaga, verba reestruturada como catálogo de regras, sindicato/dissídio, motor record-based, rateio CSC genérico, fat_folha paralela p/ conciliação; fases P1–P5) + mockup visual aprovado em `planorc-v2-postos-mockup.html`. 🟡 *(em andamento)*. **Conciliação da folha** (`/postos/conciliacao`, duas abas): *Orçado × Folha* por posto, e *Contábil × Folha* (v3_080/081/082) — que separa o razão em **três origens**: (1) o que a contabilização da folha lançou (histórico começa com código de verba da competência — concilia verba a verba e abre por funcionário), (2) **PJ** (v3_083: chega por NF em lote de contas a pagar; `posto_fornecedor` é o de-para participante×fornecedor, e `planorc_pj_casa` liga o histórico `<FORNECEDOR>-<PARTICIPANTE>` à matrícula — importado em Estrutura → Fornecedores (PJ), direto do export RD0×SA2, sem conversor), (3) o resíduo, que pede justificativa escrita (`conciliacao_folha_nota`). Tolerância configurável por tenant. **Referência do relatório** (`lib/refRelatorio.ts`): as duas abas compartilham um seletor de **relatório + linhas** (lembrado em `localStorage`); a seleção recorta o universo (masters da subárvore → contas) e traz a coluna "no relatório", calculada por `totaisRelatorio` — a mesma função do editor e dos Indicadores, com rollup de `SOMAR_FILHOS` e fórmulas. Os números não batem com a folha de propósito: o orçado da DRE está rateado no CC de destino e o da folha na origem do posto; o realizado da DRE vem do razão. ⚠ A extração do histórico é regra desta instalação do Protheus e está isolada em `planorc_pj_frag()` para ser trocada por perfil depois.
